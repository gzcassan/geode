...
public class SerialGatewaySenderEventProcessor extends AbstractGatewaySenderEventProcessor {
  ...
  public void sendBatchDestroyOperationForDroppedEvent(EntryEventImpl event) {
    if (logger.isDebugEnabled()) {
      logger.debug(
    ...
    EntryEventImpl droppedEvent = EntryEventImpl.create((LocalRegion) this.queue.getRegion(),
        Operation.DESTROY, 0, null/* newValue */, null, false, sender.getCache().getMyId());
    droppedEvent.disallowOffHeapValues();
    droppedEvent.setTailKey(-1L);

    BatchDestroyOperation op = new BatchDestroyOperation(droppedEvent);
    op.distribute();
    if (logger.isDebugEnabled()) {
      logger.debug("BatchRemovalThread completed destroy of dropped event {}", event);
    }
  }
  ...
  protected void registerEventDroppedInPrimaryQueue(EntryEventImpl droppedEvent) {
    ...
    sendBatchDestroyOperationForDroppedEvent(droppedEvent);
  }
}
